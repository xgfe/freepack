...
function Packer(option) {
    ...
    this.src = path.resolve(option.context, option.src);
    this.output = path.resolve(option.context, option.output);
    this.backup = option.backup ? path.resolve(option.context, String(option.backup).replace(/(\[([^\]]+)\])/g, (...args) => {
...
Packer.prototype.createStable = function() {
    ...
    if (/^git:?/.test(option.diff)) {
        const git_dir = path.join(this.cache, 'stable');
        createDiff(
            git_dir,
            option.context,
            option.diff.replace(/^git:?/, '')
        );
        stable_dir = path.join(git_dir, option.src);
    } else {
        stable_dir = path.resolve(option.context, option.diff.replace(/^path:?/, ''));
    }
...
Packer.prototype.match = function() {
    ...
    const baseDir = path.resolve(option.context, option.output);
