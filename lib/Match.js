        ...
        if (ruleType === 'string') {
            ...
            Object.keys(rule).map(key => {
                ...
                if (typeof value !== 'boolean') {
                    // replace "module|regexp|match" to string
                    key = key
                    .replace(new RegExp(
...
Match.prototype.test = function (path) {
    ...
    this.rules.forEach(rule => {
        ...
        switch (stat) {
            case VAR.TEST_STAT.RELEASE:
            case VAR.TEST_STAT.UNRELEASE_MATCHED:
                if (matched) {
                    switch (matchMode) {
                        case VAR.MATCH_MODE.WARN:
                            console.warn(`path[${path}] match repeat by rule[${rule.toString()}]`);
                            break;
                        case VAR.MATCH_MODE.STRICT:
                            throw new Error(`path[${path}] must match once, repeat by rule[${rule.toString()}]`);
                    }
                }
                matched = true;
                release = stat === VAR.TEST_STAT.RELEASE;
                break;
        }
    });
    return matched
        ? release
            ? VAR.TEST_STAT.RELEASE
            : VAR.TEST_STAT.UNRELEASE_MATCHED
        : VAR.TEST_STAT.UNRELEASE_UNMATCHED
};
